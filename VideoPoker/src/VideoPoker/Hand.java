...
class Hand { // packaged class
	...
	public Hand(Deck initialDeck){ // Constructor (should only be called once)
		Hand_init(initialDeck);
	}
	
	public void Hand_init(Deck initialDeck){ // Everytime we want a new hand call this (same object but different cards)
		...
		*/
			for(int i=0;i<10;i++){
				if(i<5) currentCards[i]=initialDeck.GetCard(n[i]-1);
				else hiddenCards[i-5]=initialDeck.GetCard(n[i]-1);
		}
